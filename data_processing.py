...
class Navigator:
    ...
    ## These should probably just be free standing functions instead of in the class
    def make_dir(self,dir_name):
        if not os.path.exists(dir_name):
            os.mkdir(dir_name)

    def get_fns(self,dir_name,pattern):
        return np.sort(glob.glob(os.path.join(dir_name, pattern)))

    def load_traj_coords_dir(self,dir_name,pattern,top):
    ...
    def load_npy_dir(self,dir_name,pattern):
        fns = get_fns(dir_name, pattern)
        all_d = []
        for fn in fns:
            d = np.load(fn)
...
class ProcessTraj:
    ...
    def make_master_pdb(self):
        ...
        pdb_fn = self.myNav.var_pdb_fns[0]
    ...
    def make_traj_list(self):
        ...
        for vd, fn in zip(var_dir_names,var_pdb_fns):
            ...
            for traj_fn in traj_fns:
                #i indicates which variant the traj came from
    ...
    def _preprocess_traj(self,inputs):
        ...
        v = self.myNav.var_dir_names[var_ind]
    ...
    def preprocess_traj(inputs,xtc_dir,indicator_dir):
        ...
        traj_fns = get_fns(xtc_dir, "*.xtc")
        cm_fns = get_fns(xtc_dir, "cm*.npy")
        n_traj = len(traj_fns)
...
class WhitenTraj: 
    ...
    def run(self):
        ...
        n_cores = mp.cpu_count()
        traj_fns = nnutils.get_fns(self.myNav.xtc_dir, "*.xtc")
